...
router.route('blogs/*blog/*article', function() {
  /**
   * -------------------------
...
router.route('blogs/*blog', function() {
});
...
router.route('collections/*type', function() {
  ...
  (function() {
    ...
    var bindFilters = function() {
      $('#collection-sort-by').val(window.theme.collectionSortBy).on('change', function () {
  ...
  (function() {
    ...
    $(document).on('shopify:section:unload', '#shopify-section-collection-template', function(event) {
      $(event.target).find('#instafeed').data('plugin_trademarkInstagram').destroy();
    });

    $(document).on('shopify:section:load', '#shopify-section-collection-template', function(event) {
...
router.route('gift_cards/*id', function() {
  (function() {
    var config = {
      qrCode: '#QrCode',
...
router.route('', function() {
  ...
  (function() {
    var initFullScreenSlideshow = function(slideshow) {
      ...
      var players = plyr.setup(slideshow.find('.plyr-video[data-type]'), {
          controls: ['play-large'],
          iconPrefix: 'icon-video'
        }) || [];
      ...
      players.forEach(function(player) {
        player.on('play', function() {
          slideshow.closest('.fs-slideshow').addClass('fs-slideshow--video-playing');
          slideshow.slick('pause');
        });

        player.on('pause ended', function() {
          slideshow.closest('.fs-slideshow').removeClass('fs-slideshow--video-playing');
          slideshow.slick('play');
        });
      });

      slideshow.on('beforeChange', function(event, slick, currentSlide, nextSlide) {
        // We need to pause any video if any is playing
        players.forEach(function(player) {
          player.pause();
        });
      });
  ...
  (function() {
    ...
    $(document).on('shopify:section:unload', '.shopify-section__instagram', function(event) {
      $(event.target).find('#instafeed').data('plugin_trademarkInstagram').destroy();
    });
...
router.route('products_preview', productRoute);